...
export const Hero: React.FC = () => {
  const { t, centers, queueTokens } = useAuth();
  ...
  const tokenNumber =
    farmerToken?.tokenNumber !== undefined
  ...
  const cropName =
    farmerToken?.crop ||
    farmerToken?.cropName ||
    "Your Crop";

  const tokenStatus = farmerToken?.status || "upcoming";

  const peopleAhead =
    farmerToken?.peopleAhead ??
    farmerToken?.ahead ??
    null;

  const estimatedWait =
    farmerToken?.estimatedWait ??
    farmerToken?.waitTime ??
    null;

  const bookingDate =
    farmerToken?.date ||
    farmerToken?.bookingDate ||
    null;

  const bookingTime =
    farmerToken?.time ||
    farmerToken?.slot ||
    farmerToken?.timeSlot ||
    null;
  ...
  return (
    ...
    >
      ...
      <div className="pointer-events-none absolute inset-0">
        ...
        {/* Green glow */}
        <div className="absolute -right-40 -top-40 h-[520px] w-[520px] rounded-full bg-emerald-200/30 blur-[120px]" />

        <div className="absolute -bottom-52 -left-40 h-[500px] w-[500px] rounded-full bg-lime-200/20 blur-[120px]" />
                ...
                {hasActiveToken ? (
                  ...
                  <>
                    ...
                    <p className="mt-3 text-xs leading-5 text-slate-500">
                      Book your procurement slot and receive a digital token before visiting the centre.
                    </p>
      ...
      <div className="relative border-t border-slate-200/80 bg-white/75 backdrop-blur-xl">
        ...
        <div className="mx-auto grid max-w-[1450px] grid-cols-1 sm:grid-cols-3">
